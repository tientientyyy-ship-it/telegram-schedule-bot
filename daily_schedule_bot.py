...
# 🔥 SYNCHRONOUS SQLite (Railway stable)
def init_db():
...
async def start(event):
    await event.reply("🤖 **Schedule Bot 24/7** ✅ Railway OK!\n\nBot nhắc lịch **24/24**!", buttons=MAIN_MENU)
...
async def button_handler(event):
    ...
    try:
        if data == "today":
            ...
            await event.reply("📝 **Thêm:** `/add 09:00 Task`", buttons=MAIN_MENU)
        elif data == "done_menu":
            await event.reply("✅ **Done:** `/done 08:00`", buttons=MAIN_MENU)
        elif data == "settings":
            await event.reply("⚙️ **Commands:**\n`/schedule` `/add` `/del 08:00` `/reset`", buttons=MAIN_MENU)
        elif data == "reset":
